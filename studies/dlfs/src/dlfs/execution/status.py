...
from repro_core.execution import RunPlan
...
def inspect_plan_status(
    ...
    *,
    ...
    for plan in plans:
        ...
        matches = indexed.get((plan.experiment_id, plan.atomic_run_id, seed), [])
        entries.append(_classify(plan, matches, variant=variant, seed=seed))
...
def _protocol_compatible(
    ...
    schema = importlib.import_module(f"dlfs.{volume.value}.result_schema")
