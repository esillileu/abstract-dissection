...
TRIAL_IDS = {
    ...
    "e02": tuple(
        f"dlfs1.ch06.init-compare.{name}" for name in ("std-001", "xavier", "he")
    ),
    "e03": (
    ...
    "e13": ("dlfs1.ch05.two-layer-net.backprop",),
}
...
def _metrics(experiment: str, rows: list[dict[str, str]]) -> list[OriginalMetric]:
    if experiment in {"e01", "e02"}:
        ...
        return (
            []
            if value is None
            else [OriginalMetric("final_train_objective", value, "raw", 3)]
        )
    ...
    if experiment == "e05":
        ...
        return (
            []
            if value is None
            else [OriginalMetric("final_train_accuracy", value, "percent", 2, 100.0)]
        )
