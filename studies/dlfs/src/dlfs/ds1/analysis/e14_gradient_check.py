        ...
        if rows:
            ...
            for row in rows:
                if "method" in row:
                    ...
                    values[str(row["metric"]).removeprefix("gradient_check/")] = float(
                        row["value"]
                    )
            if values:
                return values
...
def render(data, error_style, output):
    ...
    if any(differences.values()):
        ...
        stds = [
            np.std(differences[name], ddof=1) if len(differences[name]) > 1 else 0.0
    ...
    with summary.open("w", newline="", encoding="utf-8") as stream:
        writer = csv.DictWriter(
            stream,
            fieldnames=("kind", "name", "mean", "standard_deviation", "run_count"),
        )
        writer.writeheader()
        for kind, values in (
            ("mean_absolute_difference", differences),
            ("timing", timings),
        ):
