...
def test_gt03_runspec_declares_documented_offset_update_cadence() -> None:
    spec = parse_run_spec(
        "studies/dlfs/src/dlfs/ds1/config/implemented/e03_mnist_weight_decay.yaml",
        atomic_run_id="REG-WD-OFF",
    )
...
def test_gt05_runspec_declares_documented_train_probe_cadence() -> None:
    spec = parse_run_spec(
        "studies/dlfs/src/dlfs/ds1/config/implemented/e05_mnist_batchnorm_scale.yaml",
        atomic_run_id="BN-SCALE-01-ON",
    )
...
def test_gt06_and_gt07_runspec_declare_fixed_first_1000_sources() -> None:
    for path, atomic_run_id, group_id in (
        (
            "studies/dlfs/src/dlfs/ds1/config/implemented/e06_mnist_simple_cnn.yaml",
            "CNN-SIMPLE-BOOK",
            "GT06",
        ),
        (
            "studies/dlfs/src/dlfs/ds1/config/implemented/e07_mnist_deep_cnn.yaml",
            "CNN-DEEP-BOOK",
            "GT07",
        ),
    ):
...
def test_gt08_runspec_declares_20_update_train_and_test_cadence() -> None:
    spec = parse_run_spec(
        "studies/dlfs/src/dlfs/ds1/config/implemented/e08_mnist_spatial_layout.yaml",
    ...
    assert (spec.triggers[0].start, spec.triggers[0].every, spec.triggers[0].stop) == (
        20,
        20,
        None,
    )
    assert spec.triggers[0].sources == (
...
def test_gt11_reuses_gt07_protocol_for_two_model_variants() -> None:
    ...
    for atomic_run_id in ("SIMPLE-CNN-HE", "TWO-LAYER-NET-ADAM", "MLP-EXT-NO-REG"):
        spec = parse_run_spec(
            "studies/dlfs/src/dlfs/ds1/config/implemented/e15_mnist_model_comparison.yaml",
