...
from dlfs.ds1.analysis import e09_optimizer_trajectory
...
def test_optimizer_trajectories_are_stacked_in_book_order(monkeypatch) -> None:
    ...
    assert all(
        ...
        for upper, lower in itertools.pairwise(plot_axes)
    )
    assert all(
    ...
    assert all(
        max(line.get_zorder() for line in axis.lines) == 20 for axis in plot_axes
    )
    plt.close(figure)
