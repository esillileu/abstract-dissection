...
| `GT02` | PTB Word2Vec | PTB train, window 5, hidden 100, Adam .001, batch 100, 10 epochs, CUDA | architecture × objective | 4 |
...
| --- | --- | --- | --- |
| `W2V-PTB-CBOW-NS` | Dumb CBOW, window 5, hidden 100 | embedding lookup | non-fused negative sampling, 5 negatives |
| `W2V-PTB-SKIPGRAM-NS` | Dumb Skip-gram, window 5, hidden 100 | embedding lookup, independent context outputs | non-fused negative sampling, 5 negatives |
| `W2V-PTB-CBOW-FULL` | Dumb CBOW, window 5, hidden 100 | embedding lookup | two-shard full vocabulary softmax |
| `W2V-PTB-SKIPGRAM-FULL` | Dumb Skip-gram, window 5, hidden 100 | pair-expanded embedding lookup | two-shard full vocabulary softmax |
...
| `LM-BETTER-NODROPOUT` | BetterRnnlm, two-layer LSTM 650/650, dropout disabled (ratio 0) | best valid PPL; non-improvement → lr / 4 |
