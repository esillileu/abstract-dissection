...
def _matrices(client, run_refs):
    ...
    for run in run_refs:
        ...
        for row in artifact_rows(client, run, "observations/attention.csv"):
            try:
                cells[row["example_id"]][
                    (int(row["decode_step"]), int(row["encoder_position"]))
                ] = float(row["weight"])
...
def render(client, error_style, output):
    ...
    for condition in conditions:
        ...
        for example, (mean, _minimum, _maximum, _count) in sorted(matrices.items()):
            ...
            source_labels = (
                metadata.get("source_labels") if isinstance(metadata, dict) else None
            )
            target_labels = (
                metadata.get("target_labels") if isinstance(metadata, dict) else None
            )
            if source_labels:
            ...
            axis.invert_yaxis()
            axis.set(
                xlabel="encoder character position", ylabel="decoder character position"
            )
