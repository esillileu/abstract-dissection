...
from dlfs.ds2.analysis import e05_lm_recipes
...
def test_e05_main_graph_is_better_train_and_validation(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        e05_lm_recipes,
        "_valid_curves",
        lambda _client, atomic_ids: {f"{atomic}/valid": curve for atomic in atomic_ids},
    )
    monkeypatch.setattr(
        e05_lm_recipes,
        "_train_curves",
        lambda _client, atomic_ids: {f"{atomic}/train": curve for atomic in atomic_ids},
    )
...
def test_e05_original_main_graph_is_validation_only(monkeypatch, tmp_path) -> None:
    ...
    figure, curves = e05_lm_recipes.render(data, "band", tmp_path / "ds2_e05_org.png")
...
def test_e05_better_graph_is_single_axis_without_wave_break(
