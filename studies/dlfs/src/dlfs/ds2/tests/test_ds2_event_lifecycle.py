...
def test_ds2_records_long_metrics_and_source_samples() -> None:
    ...
    executor = EventExperimentExecutor(
        ...
        source_curve=lambda event: {
            "series_id": "book-ppl",
            "plot_index": event.local_iteration,
    ...
    assert {row["metric"] for row in executor.records.evaluations} == {
        "loss",
        "perplexity",
    }
...
def test_ds2_source_curve_reduces_standard_and_book_losses_together() -> None:
    ...
    assert first["book_value"].backend.scalar_to_float(first["book_value"].data) == 6.0
...
def test_ds2_source_curves_csv_schema_and_mlflow_mapping(tmp_path) -> None:
    ...
    assert (
        0,
        "series/eval_test/exact_match_accuracy",
        0.42,
    ) in records.mlflow_metric_rows()
...
def test_ds2_writes_canonical_source_objectives_and_prediction_fields(tmp_path) -> None:
    ...
    predictions = list(
        csv.DictReader((tmp_path / "observations/predictions.csv").open())
    )
    assert objectives == [
        {
            "update": "1",
