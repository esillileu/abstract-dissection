...
from dlfs.ds2.analysis import e10_word2vec_profile
...
CONFIG = Path(
    "studies/dlfs/src/dlfs/ds2/config/implemented/e10_ptb_word2vec_profile.yaml"
)
...
def test_e10_pf01_expands_to_atomic_single_profile_runs() -> None:
    ...
    spec = parse_run_spec(CONFIG, atomic_run_id="PF-W2V-CBOW-ORIGINAL-NS")
...
def test_e10_analysis_uses_implemented_profiles_and_short_labels() -> None:
    ...
    assert all(
        "-IMPLEMENTED-" in atomic_id for atomic_id in e10_word2vec_profile.ATOMIC_IDS
    )
    assert all(
        "-ONEHOT-" not in atomic_id for atomic_id in e10_word2vec_profile.ATOMIC_IDS
    )
