...
def test_word2vec_runspec_connects_source_curve_contract() -> None:
    spec = parse_run_spec(
        "studies/dlfs/src/dlfs/ds2/config/implemented/e02_ptb_word2vec.yaml",
...
def test_toy_skipgram_matches_cbow_conditions_except_architecture() -> None:
    cbow = parse_run_spec(
        "studies/dlfs/src/dlfs/ds2/config/implemented/e01_toy_word2vec.yaml",
...
def test_lm_recipe_runspec_declares_valid_selected_checkpoint_inputs() -> None:
    spec = parse_run_spec(
        ...
        atomic_run_id="LM-BETTER-RECIPE",
    )
...
def test_seq2seq_runspec_declares_predictions_and_attention_observation_inputs() -> (
    None
):
    spec = parse_run_spec(
        "studies/dlfs/src/dlfs/ds2/config/implemented/e07_date_seq2seq.yaml",
        atomic_run_id="SEQD-ATTN-REV",
    )

    assert spec.identity.group_id == "GT07"
    assert spec.recording["predictions"] == {
        "split": "test",
        "count": 10,
        "decode": "greedy",
    }
...
def test_addition_seq2seq_runspec_declares_batched_epoch_evaluation() -> None:
    spec = parse_run_spec(
        "studies/dlfs/src/dlfs/ds2/config/implemented/e09_addition_seq2seq_150.yaml",
...
def test_e06_addition_seq2seq_drops_the_source_remainder_batch() -> None:
    spec = parse_run_spec(
        "studies/dlfs/src/dlfs/ds2/config/implemented/e06_addition_seq2seq.yaml",
        atomic_run_id="SEQA-VAN-FWD",
    )
...
def test_gt03_catalog_excludes_the_custom_loop_variant() -> None:
    with pytest.raises(ValueError, match="unknown atomic_run_id"):
        parse_run_spec(
            "studies/dlfs/src/dlfs/ds2/config/implemented/e03_small_rnnlm.yaml",
