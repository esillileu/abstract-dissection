...
from dlfs.ds2.original.run import e12 as original_e12
...
CONFIG = Path(
    "studies/dlfs/src/dlfs/ds2/config/implemented/e12_count_based_embeddings.yaml"
)
...
def test_count_based_pipeline_is_numerically_reusable_and_seeded() -> None:
    ...
    full, singular_values, _ = factorize_ppmi(ppmi, method="svd", components=2, seed=1)
...
def test_original_e12_records_reusable_matrices_and_phase_times(
    ...
    assert timing["total_s"] == (
        timing["cooccurrence_s"] + timing["ppmi_s"] + timing["decomposition_s"]
    )
