    ...
    def to_executor_config(self) -> dict[str, object]:
        ...
        if self.source_curve is not None:
            recording["source_curve"] = {
                ...
                **(
                    {}
                    if self.source_curve.every_updates is None
                    else {"every_updates": self.source_curve.every_updates}
                ),
                **(
                    {}
                    if self.source_curve.every_epochs is None
                    else {"every_epochs": self.source_curve.every_epochs}
                ),
        ...
        return {
            ...
            "policy": {
                ...
                **(
                    {"max_grad": self.seed_policy["max_grad"]}
                    if "max_grad" in self.seed_policy
                    else {}
                ),
                **{
