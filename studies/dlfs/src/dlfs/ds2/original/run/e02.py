    ...
    *,
    ...
    if kind == "cbow":
        ...
        class FullSoftmaxCBOW:
            def __init__(self):
                w_in = 0.01 * xp.random.randn(vocab_size, hidden_size).astype("f")
                w_out = 0.01 * xp.random.randn(hidden_size, vocab_size).astype("f")
                input_layer_cls = matmul_cls if one_hot else embedding_cls
                self.in_layers = [input_layer_cls(w_in) for _ in range(2 * window_size)]
                self.out_layer = matmul_cls(w_out)
                self.loss_layer = loss_cls()
                model_layers = [*self.in_layers, self.out_layer]
                self.params = [
                    param for layer in model_layers for param in layer.params
                ]
                self.grads = [grad for layer in model_layers for grad in layer.grads]
                self.word_vecs = w_in
    ...
    if kind == "skipgram":
        ...
        class FullSoftmaxSkipGram:
            def __init__(self):
                ...
                self.out_layer = matmul_cls(w_out)
                self.loss_layers = [loss_cls() for _ in range(2 * window_size)]
...
TRIALS = tuple(
    Trial(
        ...
        (
            ...
            if objective == "negative-sampling"
            else COMMON_SOURCES + MODEL_SOURCES[kind] + ("ch04/train.py",)
        ),
