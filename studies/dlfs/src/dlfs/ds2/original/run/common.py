...
from dlfs.original_runtime.cache_protocol import load_npz, save_csv, save_npz, to_host
from dlfs.original_runtime.runtime_context import array_module, device
...
def checkpoint_arrays(params: list[object]) -> dict[str, np.ndarray]:
    return {f"param_{index:03d}": to_host(param) for index, param in enumerate(params)}
