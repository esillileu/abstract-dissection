...
from dlfs.original_runtime.summary import (
    OriginalMetric,
    last_value,
...
TRIAL_IDS = {
    ...
    "e12": tuple(
        f"dlfs2.ch02.ptb-{name}" for name in ("ppmi", "svd", "randomized-svd")
    ),
}
...
def _metrics(experiment: str, rows: list[dict[str, str]]) -> list[OriginalMetric]:
    if experiment in {"e01", "e02"}:
        ...
        return [] if value is None else [OriginalMetric("final_loss", value, "raw", 3)]
    if experiment == "e12":
        metrics = []
        for name in ("cooccurrence_s", "ppmi_s", "decomposition_s", "total_s"):
            values = [
                float(row["value"])
                for row in rows
    ...
    if experiment == "e03":
        ...
        return (
            []
            if value is None
            else [OriginalMetric("final_train_perplexity", value, "raw", 2)]
        )
    if experiment == "e04":
        metrics = []
    ...
    if experiment in {"e06", "e07"}:
        ...
        return (
            []
            if value is None
            else [OriginalMetric("final_test_accuracy", value, "percent", 2, 100.0)]
        )
