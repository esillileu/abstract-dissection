    ...
    for index, scope in enumerate(
        ...
        for component_index, component in enumerate(components):
            ...
            axis.bar(
                ...
                color=ACCENT_COLORS[(component_index + index * 3) % len(ACCENT_COLORS)],
            )
            bottoms = [
                left + right for left, right in zip(bottoms, values, strict=False)
            ]

    for condition, total in zip(CONDITION_ORDER, bottoms, strict=False):
