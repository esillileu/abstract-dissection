...
from deepscratch.datasets import load_sequence
from deepscratch.nn.model.architecture import PeekySeq2seq, Seq2seq
from deepscratch.nn.objective import TemporalSoftmaxCrossEntropy
...
class Workload:
    def __init__(self, backend, *, model_class, reverse: bool) -> None:
        ...
        self.position = 0
        self.batch_size = 128
        self.model = model_class(len(data["char_to_id"]), 16, 128, backend=backend)
        ...
        parameters = [
            ...
            *(
                ...
                for name, value in self.objective.named_parameters()
            ),
        ]
        self.optimizer = Adam(parameters, lr=0.001, pre_step_hooks=[ClipGradNorm(5.0)])
    ...
    for name in selected_conditions:
        ...
        for _ in range(repetitions):
            ...
            for _ in range(iterations):
                elapsed = _timed(
                    backend,
                    lambda losses=losses, workload=workload: losses.append(
                        workload.update()
                    ),
                )
        ...
        result["conditions"][name] = {
            ...
            "parameter_count": sum(
                parameter.data.size
                for _, parameter in workload.model.named_parameters()
            ),
