...
from dlfs.original_runtime.cache_protocol import cache_is_valid, load_csv
...
def write_experiment_summary(
    *,
    ...
    for trial_id in trial_ids:
        ...
        backend = (
            "unknown" if manifest is None else str(manifest.get("backend", "unknown"))
        )
        ...
        parameter_manifest = _json_object(directory / "parameter_manifest.json")
...
def _optional_scalar_row(
    *,
    ...
    return {
        ...
        "unit": unit,
        "mean": text,
        "standard_deviation": (f"{0.0:.{decimals}f}" if standard_deviation else ""),
        "minimum": text,
        "maximum": text,
    }
