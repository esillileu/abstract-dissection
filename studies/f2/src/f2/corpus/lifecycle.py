...
from .sources import SOURCES, VALIDATION_PROFILES, CorpusSource, stable_id
...
def process_source(
    ...
    *,
    ...
    if normalized:
        records = ((identity, normalize_text(text)) for identity, text in records)
    sharder = DeterministicSharder(output_dir, target_words)
    shards = sharder.write(records, source=source.key)
