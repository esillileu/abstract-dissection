"""Tests for CleanTextWriter and storage helpers."""

from __future__ import annotations

from pathlib import Path

from f2.storage import CleanTextWriter
...
def test_clean_text_writer(tmp_path: Path):
    ...
    sha1, p1 = writer.write_document(
        "Hello world news article.", 4, "http://test.com/1"
    )
    sha2, p2 = writer.write_document(
        "Second article body text.", 4, "http://test.com/2"
    )
    writer.close()
