...
def f2_test_database_url():
    ...
    url = os.environ.get("F2_CC_TEST_DATABASE_URL")
    container = None
    engine = None
    try:
        if not url:
            ...
            url = f"postgresql://postgres:f2-test-only@127.0.0.1:{port}/f2_cc_test"
...
def f2_test_database(f2_test_database_url, monkeypatch):
    ...
    monkeypatch.setenv("F2_CC_DATABASE_URL", f2_test_database_url)
