...
from repro_core.context.progress import NullProgressReporter, ProgressReporter
from repro_core.numerics.device import DeviceTimer, DeviceTimingToken, NullDeviceTimer
...
class EventExperimentExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        update_requests: Callable[[UpdateEvent], tuple[EvaluationRequest, ...]]
        | None = None,
        epoch_requests: Callable[[EpochEvent], tuple[EvaluationRequest, ...]]
        | None = None,
        terminal_requests: Callable[[TrainEndEvent], tuple[EvaluationRequest, ...]]
        | None = None,
        source_curve: Callable[[SourceObjectiveSample], dict[str, object] | None]
        | None = None,
        after_evaluation: Callable[[EvaluationRequest, object, str, int], None]
        | None = None,
        ...
        self._after_evaluation = after_evaluation or (
            lambda _request, _result, _axis, _step: None
        )
    ...
    def on_train_end(self, event: TrainEndEvent) -> None:
        ...
        if (
            self._window_start_update is not None
            and self._window_start_update <= event.update
        ):
            self._close_window(
                end_update=event.update,
                closed_by="terminal",
        ...
        *,
        ...
        if self._window_start_update > end_update:
            self._record_evaluations(
                ...
                axis_step=epoch if closed_by == "epoch_end" else end_update,
                update=end_update,
                epoch=epoch,
            )
            return
        ...
        self._record_evaluations(
            requests,
            axis=axis,
            axis_step=epoch if axis == "epoch" else end_update,
            update=end_update,
            epoch=epoch,
        )
