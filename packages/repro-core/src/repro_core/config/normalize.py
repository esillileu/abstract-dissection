"""Portable, YAML-facing experiment configuration contract and normalizer."""
...
def normalize_config(config: dict[str, object]) -> dict[str, object]:
    ...
    value["atomic_run_id"] = str(
        value.get("atomic_run_id", metadata.get("atomic_run_id", value["kind"]))
    )
    experiment_ids = value.get(
        "experiment_ids", metadata.get("experiment_ids", (value["kind"],))
    )
    ...
    value["execution_group_id"] = str(
        value.get(
            "execution_group_id", metadata.get("execution_group_id", value["kind"])
        )
    )
    value["recipe_id"] = str(
        value.get("recipe_id", metadata.get("recipe_id", value["kind"]))
    )
    value["structure_signature"] = str(
        value.get(
            "structure_signature", metadata.get("structure_signature", value["kind"])
        )
    )
    ...
    training.setdefault(
        "entrypoint", str(value.get("entrypoint", f"yaml:{value['kind']}"))
    )
