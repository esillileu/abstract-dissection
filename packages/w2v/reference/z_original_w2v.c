...
void read_word(char *word, FILE *fin, char *eof) {
  int a = 0, ch;
  while (1) {
...
int read_word_index(FILE *fin, char *eof) {
  char word[MAX_STRING], eof_l = 0;
  read_word(word, fin, &eof_l);
  if (eof_l) {
    *eof = 1;
    return -1;
  }
...
int add_world_to_vocab(char *word) {
...
void build_vocab_from_file() {
  ...
  vocab_size = 0;
  add_world_to_vocab((char *)"</s>");
  while (1) {
    read_word(word, fin, &eof);
    if (eof) break;
    train_words++;
    wc++;
...
void TrainModel() {
  ...
  if (read_vocab_file[0] != 0) read_vocab(); else build_vocab_from_file();
