...
from repro_mlflow.runtime import (
    RuntimeOptions,
...
def test_latest_and_best_checkpoint_uploads_are_enabled_by_default() -> None:
    options = RuntimeOptions(
        tracking_uri="http://example.invalid", experiment_name="example"
    )

    assert options.upload_checkpoint is True
    assert options.upload_eval_checkpoints is True
...
def test_latest_and_best_checkpoint_upload_by_default_and_honor_opt_out(
    ...
    assert client.files == []
    assert client.trees == [
        ("run-id", str(final_path), f"checkpoints/generations/{final_path.name}"),
        ("run-id", str(eval_path), f"checkpoints/generations/{eval_path.name}"),
    ]

    disabled_client = _ArtifactClient()
    disabled_sink = _Sink(
        RuntimeOptions(
            tracking_uri="http://example.invalid",
            experiment_name="example",
            upload_eval_checkpoints=False,
        ),
        "run",
        {},
        {},
    )
    disabled_sink.run_id = "run-id"
    monkeypatch.setattr(disabled_sink, "_start_mlflow", lambda: disabled_client)
    disabled_sink.put(("checkpoint", (eval_path, "eval")))
    disabled_sink.put(("stop", "FINISHED"))

    disabled_sink._consume()

    assert disabled_client.trees == []


def test_artifact_upload_uses_no_artifact_path_for_root_files(
    tmp_path, monkeypatch
) -> None:
...
def test_condition_parent_is_created_once_per_condition_key() -> None:
    ...
    child_tags = {
        "run.type": "seed_trial",
        "condition.key": "condition-abc",
        "atomic_run.id": "MLP-ADAM-HE",
        "run.key": "seed-1",
        "master_seed": "1",
        "trial.status": "running",
    }

    first = get_or_create_condition_parent(
        client, experiment_id="1", child_tags=child_tags
    )
    second = get_or_create_condition_parent(
        client, experiment_id="1", child_tags=child_tags
    )
    ...
    assert client.created_tags == {
        ...
        "condition.group.key": "condition-abc",
        "mlflow.runName": "MLP-ADAM-HE",
    }
