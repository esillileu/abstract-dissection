...
def test_artifact_cache_separates_mlflow_stores_with_equal_run_ids(
    tmp_path: Path,
) -> None:
    cache = tmp_path / "shared-artifacts"
    clients = (_Client(b"one-"), _Client(b"two-"))
    paths = []
    for uri, client in zip(
        ("sqlite:///one.db", "sqlite:///two.db"), clients, strict=True
    ):
        data = StudyAnalysisInput(
            client,
            None,
