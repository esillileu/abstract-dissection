...
def test_flatten_dict_uses_slash_keys() -> None:
    assert flatten_dict({"model": {"name": "SimpleCNN"}}) == {"model/name": "SimpleCNN"}
...
def test_parent_group_key_distinguishes_reused_atomic_run_names() -> None:
    ...
    initializer_tags = build_tags(
        initializer_identity, {"kind": "test"}, git_info, None
    )

    assert (
        optimizer_tags["condition.group.key"] != initializer_tags["condition.group.key"]
    )
