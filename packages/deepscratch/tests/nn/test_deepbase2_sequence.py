...
from deepscratch.nn.layers import TimeAttention
from deepscratch.nn.model import TimeSequential
...
def test_word2vec_model_owns_both_embedding_matrices() -> None:
    ...
    assert [
        (name, parameter.data.shape) for name, parameter in model.named_parameters()
    ] == [
        ("W_in", (7, 5)),
        ("W_out", (7, 5)),
    ]
