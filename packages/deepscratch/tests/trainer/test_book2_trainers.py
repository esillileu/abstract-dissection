...
from deepscratch.trainer import LanguageModelTrainer, Seq2seqTrainer, Word2VecTrainer
...
def test_seq2seq_trainer_evaluates_greedy_exact_match() -> None:
    ...
    result = trainer.evaluate(
        xs, ts, metrics=("exact_match_accuracy", "token_accuracy")
    )
...
def test_seq2seq_evaluation_decodes_in_configured_batches() -> None:
    ...
    def generate_batch(question, _start_id, sample_size):
        ...
        return question.backend.xp.zeros(
            (len(question), sample_size), dtype=question.backend.xp.int64
        )
