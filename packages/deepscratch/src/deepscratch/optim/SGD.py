...
if TYPE_CHECKING:
    from deepscratch.core.backend.types import Array
    from deepscratch.nn.types import NamedParameters
...
class SparseAdam(Adam):
    ...
    def __init__(
        ...
        *,
        ...
        if unknown:
            raise ValueError(
                f"sparse Adam row providers have unknown parameters: {sorted(unknown)}"
            )
