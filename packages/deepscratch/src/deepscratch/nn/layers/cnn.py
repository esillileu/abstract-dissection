...
from deepscratch.nn.types.parameter import Parameter

from ..utils.cnn import _calculate_output_size, _pair, col2im, im2col
...
class Conv2D(Layer):
    ...
    def forward_manual(self, x: Tensor) -> Tensor:
        ...
        requires_grad = (
            ...
            or (self.b is not None and self.b.requires_grad)
        )
    ...
    def backward_manual(self, dout: Tensor) -> Tensor:
        ...
        if dout.backend.device != self.backend.device:
            raise ValueError("dout and Conv2D parameters must be on the same device")
    ...
    def _validate_input(self, x: Tensor) -> None:
        if x.ndim != 4:
            raise ValueError(
                f"Conv2D input must have shape (N, C, H, W), got {x.shape}"
            )
        ...
        if x.backend.device != self.backend.device:
            raise ValueError("input and Conv2D parameters must be on the same device")
