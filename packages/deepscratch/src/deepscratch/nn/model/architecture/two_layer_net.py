...
from deepscratch.nn.layers import Affine, Relu
from deepscratch.nn.model.sequential import Sequential
    ...
    def numerical_gradient(self, x, target, objective) -> None:
        ...
        for _name, parameter in self.named_parameters():
            ...
            while not iterator.finished:
                ...
                plus = objective.forward(
                    self.forward(x, cache=False), target, cache=False
                ).loss
                values[index] = original - h
                minus = objective.forward(
                    self.forward(x, cache=False), target, cache=False
                ).loss
