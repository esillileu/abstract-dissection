...
class LanguageModelTrainer(EventTrainer):
            ...
            for epoch_index in range(self.epoch, self.max_epochs):
                ...
                self._emit_epoch(
                    EpochEvent(
                        ...
                        sample_count=(
                            self.iteration_in_epoch * self.batch_size * self.time_size
                        ),
                    )
                )
                self.iteration_in_epoch = 0
    ...
    def evaluate(
        ...
        *,
        ...
        try:
            ...
            if stream_length < 1:
                raise ValueError(
                    "evaluation corpus is too short for evaluator_batch_size"
                )
        ...
        return EvaluationResult(
            example_count=token_count,
            loss=float(host_metrics[0]),
            accuracy=None,
            unit="token",
    ...
    def _batch(self, xs: Tensor, ts: Tensor) -> tuple[Tensor, Tensor]:
        ...
        positions = (
            self._batch_offsets[:, None] + self.time_index + self._time_offsets[None, :]
        ) % data_size
