...
graph TD
    ...
    STUDY -->|uses infrastructure| CORE
    STUDY -->|tracks runs| MLFLOW
    STUDY -->|builds models| ENGINE
...
| **`studies/`** | Domain-specific reproduction studies and experimental protocols (`studies/dlfs`). | Orchestrates `packages/` engines and compares against `references/`. Contains study catalogs, configs, and custom analysis scripts. |
