...
| e02 full softmax vs negative sampling | ch03 `train.py` 및 ch04 `train.py`                              | 불일치/확장 | 원본 full softmax는 `You say goodbye ...` toy corpus, window=1, hidden=5, batch=3, 1,000 epoch의 `SimpleCBOW`다. 현재는 PTB/window=5/embedding=100/10 epoch에서 CBOW와 Skip-gram 각각에 full softmax와 NS를 적용한다. 이 2×2 비교 자체는 원본에 없다.                                                                |
...
| e02 CBOW/Skip-gram full softmax | 대응 원본 ch03은 toy corpus/window=1/hidden=5/batch=3/1,000 epoch이고, 현재는 PTB/window=5/embedding=100/batch=100/10 epoch다. | 책 재현용이라면 ch03 toy 조건을 별도 추가한다. 현재 조건은 architecture×objective 확장 실험으로 유지한다. |
...
| e02 full softmax/NS | PTB train; architecture `{CBOW, Skip-gram}` × output objective `{full softmax, NS}` | 병합 가능하지만 책 밖의 factorial 확장 실험이다. | 네 조건은 같은 logging/profiling 설정을 쓰고, 같은 architecture 안에서 objective를 비교한다. loss 절대값이 아니라 속도·메모리·별도 품질 지표를 비교한다. |
...
e02의 full-softmax와 negative-sampling 조건은 모두 prediction-term 합산 loss와
같은 Word2Vec trainer를 쓴다. 그래도 softmax cross-entropy와 sigmoid negative loss의
**loss 절대값**은 같은 척도가 아니므로, 비교는 속도·메모리·학습 경향 중심으로 해석해야 한다.
...
| e02               | 원본에는 PTB full-softmax 대 negative-sampling 공통 그래프가 없다. 현재는 CBOW와 Skip-gram별로 두 목적함수의 normalized loss를 seed별 집계한다.                                                                              | 같은 architecture 안에서 목적함수의 학습 경향을 보고, loss 절대값은 비교하지 않는 확장 분석이다.                                                        |
