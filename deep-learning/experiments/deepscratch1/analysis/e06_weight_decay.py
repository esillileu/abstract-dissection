...
def main() -> None:
    ...
    for atomic_run_id in ATOMIC_RUN_IDS:
        plot_curve(axis, metric_curve(mlflow_client, grouped[atomic_run_id], "book_epoch/train/accuracy"), label=f"{atomic_run_id} train", marker="o", error_bars=ERROR_BARS)
        plot_curve(axis, metric_curve(mlflow_client, grouped[atomic_run_id], "book_epoch/test/accuracy"), label=f"{atomic_run_id} test", linestyle="--", marker="s", error_bars=ERROR_BARS)
