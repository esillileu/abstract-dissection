...
ATOMIC_RUNS = {
    "MLP-SGD-STD001": {"initializer": "std:0.01", "initializer_name": "normal_std_0.01"},
    "MLP-SGD-XAVIER": {"initializer": "xavier", "initializer_name": "xavier"},
    "MLP-SGD-HE": {"initializer": "he", "initializer_name": "he"},
}
...
def build_spec(atomic_run_id: str, *, train_size: int, test_size: int, batch_size: int, max_epoch: int, profiling: dict[str, object]) -> RunSpec:
    ...
    return RunSpec(
        ...
        initializer={"name": config["initializer_name"], "seed": "seed/model_init"},
        optimizer={"name": "sgd", "learning_rate": 0.01},
        scheduler={"name": "constant"},
        loss={"name": "SoftmaxWithLoss", "reduction": "mean"},
        training={"max_epochs": max_epoch, "max_updates": 2000, "batch_size": batch_size, "log_interval_unit": "update", "log_interval": env_int("MLPROSECTION_LOG_INTERVAL", 30), "entrypoint": "experiments/run/e04_mnist_mlp_initializer.py"},
...
def main() -> None:
    args = parser("Run e04 MNIST MLP initializer atomic trial.", sorted(ATOMIC_RUNS)).parse_args()
