...
def import_legacy(
    ...
    from .legacy import import_legacy_archive
...
def storage(
    ...
    tracking_uri: Annotated[str | None, typer.Option("--tracking-uri")] = None,
    verified_mirrors: Annotated[
        bool, typer.Option("--verified-mirrors")
    ] = False,
    apply: Annotated[bool, typer.Option("--apply")] = False,
) -> None:
    """Audit storage or remove only verified local mirrors."""
    from mlflow.tracking import MlflowClient

    from .legacy import audit_storage, cleanup_verified_mirrors

    if action not in {"audit", "cleanup"}:
        raise ValueError("storage action must be audit or cleanup")
    if action == "audit" and (verified_mirrors or apply):
        raise ValueError("audit does not accept cleanup options")
    if action == "cleanup" and not verified_mirrors:
        raise ValueError("cleanup requires --verified-mirrors")
    uri = tracking_uri or os.getenv("MLFLOW_TRACKING_URI", "http://127.0.0.1:5000")
