...
def test_ds2_skipgram_runs_pair_expanded_contexts(
    monkeypatch,
    tmp_path: Path,
    atomic_run_id: str,
) -> None:
    ...
    monkeypatch.setattr(
        "exp.deepscratch.ds2.implemented.executor.load_ptb",
        lambda: {
            "train": corpus,
            "valid": corpus[:8],
            "test": corpus[:8],
...
def test_ds2_seq2seq_config_runs_one_epoch(
    ...
    monkeypatch.setattr(
        "exp.deepscratch.ds2.implemented.executor.load_sequence",
...
def test_ds2_attention_observation_loads_checkpoint_and_runs(
    ...
    spec = parse_run_spec(
        "exp/deepscratch/ds2/config/implemented/e08_attention_alignment.yaml",
