...
from mlprosection.nn.objective import FusedNegativeSampling, NegativeSampling
...
def _optimizer_errors(backend, source: Adam, target: Adam) -> dict[str, dict[str, float]]:
    return {
        ...
        for kind, case in cases.items():
            ...
            lines.append(
                ...
                "{parameter:.3e} | {state:.3e} | {status} |".format(
                    ...
                    parameter=_max_error(rows, "parameter_errors", "required_atol_rtol"),
                    state=_max_error(rows, "optimizer_state_errors", "required_atol_rtol"),
...
def validate_kind(backend, kind: str) -> dict[str, object]:
    ...
    for update, (first, second, negatives) in enumerate(_batches(kind), start=1):
        model_x, objective_t = adapter.prepare(_tensor(backend, first), _tensor(backend, second))
        ...
        gradient_errors = {
            name: _error_metrics(backend, parameter.grad, dict(fused_model.named_parameters())[name].grad)
            for name, parameter in ordinary_model.named_parameters()
        }
        loss_error = _error_metrics(backend, ordinary_result.loss.data, fused_result.loss.data)
        ordinary_optimizer.update()
        fused_optimizer.update()
        backend.synchronize()
        parameter_errors = _parameter_errors(backend, ordinary_model, fused_model)
        optimizer_errors = _optimizer_errors(backend, ordinary_optimizer, fused_optimizer)
        row_passed = (
        ...
        rows.append(
            {
                "update": update,
                "ordinary_loss": float(backend.scalar_to_float(ordinary_result.loss.data)),
...
def run(
    ...
    for device in devices:
        backend = make_backend(BackendConfig(device=device, dtype="float32", seed=20260821))
        comparisons[device] = {
