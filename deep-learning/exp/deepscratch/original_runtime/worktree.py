...
def clean_worktree(repo: Path) -> Iterator[Path]:
    ...
    subprocess.run(
        ("git", "-C", str(repo), "worktree", "add", "--quiet", "--detach", str(checkout), commit),
        check=True,
    )
    try:
...
def _seed_untracked_dataset_cache(repo: Path, checkout: Path) -> None:
    ...
    for relative in ("dataset/mnist.pkl", "dataset/ptb.train.npy", "dataset/ptb.valid.npy", "dataset/ptb.test.npy"):
