...
from .common import COMMON_SOURCES, Trial, importlib, np, save_csv, save_params, source_imports
        ...
        with measurements.training():
            for index in range(iterations):
                ...
                if index % iter_per_epoch == 0:
                    rows.extend(
                        [
                            {"update": index + 1, "epoch": int(index / iter_per_epoch) + 1, "split": "train", "accuracy": network.accuracy(x_train, t_train)},
                            {"update": index + 1, "epoch": int(index / iter_per_epoch) + 1, "split": "test", "accuracy": network.accuracy(x_test, t_test)},
                        ]
                    )
...
TRIALS = (
    Trial(
        ...
        "numpy",
        {"gradient_method": "backprop", "updates": 10000, "batch_size": 100, "learning_rate": 0.1},
