...
from mlprosection.experiment.reproducibility import configure_runtime
from mlprosection.nn.layers import BatchNormalization
from mlprosection.trainer import ForwardTrainer

from exp.deepscratch.ds1.implemented.executor import _model, _objective, _optimizer, _training_parameters
from exp.deepscratch.ds1.implemented.final_gap import (
    TARGET_RUNS,
    TRAIN_FULL_ACCURACY,
...
def resolve_run_checkpoint(client, run) -> Path:
    ...
    for artifact_path in candidates:
        try:
            downloaded = Path(
                client.download_artifacts(run.info.run_id, artifact_path)
            )
        except Exception:
            continue
