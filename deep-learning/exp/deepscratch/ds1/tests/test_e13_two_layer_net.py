...
from exp.deepscratch.ds1.analysis import e13_two_layer_net
...
def test_e13_uses_e06_plot_format_with_fraction_axis(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _client, group, atomic_ids: {
            atomic_ids[0]: ["seed"] if group == "GT10" else []
        },
    )
    monkeypatch.setattr(
        e13_two_layer_net,
    ...
    figure, curves = e13_two_layer_net.render(object(), "band", None)
    axis = figure.axes[0]
