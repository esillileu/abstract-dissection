...
def _checkpoint_projection(client, run_id: str) -> Path | None:
    root = (
        WorkspacePaths.from_environment(Path.cwd()).cache_root
        / "deepscratch"
        / "legacy-projections"
    ...
    try:
        ...
        with np.load(source, allow_pickle=False) as arrays:
            ...
            for name in arrays.files:
                ...
                if name.startswith("param__W") and value.ndim == 4:
                    projected[f"layers.{convolution_index}.W"] = value
                    convolution_index += 1
        if not projected:
            return None
    except (OSError, ValueError):
        return None
    root.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(weights_path, **projected)
    manifest_path.write_text(
        json.dumps(
            {
                "format": "legacy-projected-npz",
                "final": {
                    "path": str(weights_path.resolve()),
                    "epoch": "",
                    "update": "",
