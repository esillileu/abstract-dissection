...
from exp.domain import DomainDefinition, RunOptions, RunOrder, RunPlan, RunSelection
...
class Planner:
    ...
    def build(
        self, selection: RunSelection, options: RunOptions
    ) -> list[RunPlan]:
        ...
        if selection.atomic_run_ids and selection.excluded_atomic_run_ids:
            raise ValueError(
                "choose at most one of --atomic-run/-a or "
                "--exclude-atomic-run/-x"
            )
        ...
        seeds = self._seed_values(
            selection.seed_set or self.domain.default_seed_set
        )
        requested_seeds = parse_seed_values(
            selection.seed_values, available=seeds
        )
        ...
        for path in sorted(
            self.domain.config_root.glob("e[0-9][0-9]_*.yaml")
        ):
            ...
            selected_variants = [
                ...
                if (not included or str(item) in included)
                and str(item) not in excluded
            ]
            ...
            if mode not in {"seeded", "single"}:
                raise ValueError(
                    f"unsupported execution.mode in {path}: {mode}"
                )
            if mode == "single":
                ...
                if invalid:
                    raise ValueError(
                        f"{experiment_id} declares seed values "
                        f"{available_seeds}; invalid values: {invalid}"
                    )
        ...
        if options.order is RunOrder.SEED_FIRST:
            plans.sort(
                key=lambda plan: seed_order.get(plan.seed, len(seed_order))
    ...
    def _seed_values(self, seed_set: str) -> list[int]:
        ...
        try:
            return [
                int(value)
                for value in registry["seed_sets"][seed_set]["values"]
            ]
