...
class Planner:
    ...
    def build(self, selection: RunSelection, options: RunOptions) -> list[RunPlan]:
            ...
            for atomic_run_id in selected_variants:
                ...
                for seed in run_seeds:
                    plans.append(RunPlan(
                        ...
                        options.device or str(execution.get("default_device", "cpu")),
                    ))
        unknown_experiments = selected - matched_experiments
        if unknown_experiments:
            raise ValueError("unknown experiment in catalog: " + ", ".join(sorted(unknown_experiments)))
        unknown_atomic_runs = requested_atomic_runs - matched_atomic_runs
        if unknown_atomic_runs:
            raise ValueError("unknown atomic run ID in selected experiments: " + ", ".join(sorted(unknown_atomic_runs)))
        if not plans:
        ...
        if options.order is RunOrder.SEED_FIRST:
            plans.sort(key=lambda plan: seed_order.get(plan.seed, len(seed_order)) if plan.seed is not None else len(seed_order))
        return plans
    ...
    def _seed_values(self, seed_set: str) -> list[int]:
        ...
        try:
            ...
            raise ValueError(f"unknown seed set for {self.domain.name}: {seed_set}") from exc
