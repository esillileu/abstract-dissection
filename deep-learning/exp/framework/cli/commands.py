...
import os
import re
from exp.framework.execution.definition import ExecutionDefinition, RunOptions, RunOrder, RunSelection
from exp.framework.execution.planning import Planner
from exp.framework.execution.runner import Runner, print_plans
...
def plan_command(
    ...
    *,
    experiments: list[str], all_experiments: bool,
    atomic_runs: list[str], excluded_atomic_runs: list[str],
    seed_set: str | None, seeds: str | None, device: str | None,
    override_values: list[str], order: RunOrder,
) -> None:
    _validate_device(device)
    selection = RunSelection(
        tuple(experiments), all_experiments or not experiments,
        tuple(atomic_runs), tuple(excluded_atomic_runs), seeds, seed_set,
    )
    options = RunOptions(device=device, overrides=parse_overrides(override_values), order=order)
...
def run_command(
    ...
    *,
    ...
    atomic_runs: list[str], excluded_atomic_runs: list[str],
    seed_set: str | None, seeds: str | None, device: str | None,
    override_values: list[str], order: RunOrder, dry_run: bool,
    progress: str, progress_every: int, tracking_uri: str | None,
) -> None:
    _validate_device(device)
    ...
    selection = RunSelection(
        tuple(experiments), all_experiments, tuple(atomic_runs),
        tuple(excluded_atomic_runs), seeds, seed_set,
    )
    options = RunOptions(
        device=device,
