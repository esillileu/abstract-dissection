...
from exp.ds2.original.run import e02
from exp.ds2.original.run.common import source_imports
...
def test_original_e02_registers_ns_and_full_softmax_trials() -> None:
    ...
    assert cbow_ns.source_files == e02.COMMON_SOURCES + (
        "ch04/cbow.py",
        "ch04/skip_gram.py",
...
def test_original_e02_full_softmax_adaptations_forward_and_backward() -> None:
    ...
    with source_imports(BOOK_ROOT):
        for kind in ("cbow", "skipgram"):
            model = e02.build_full_softmax_model(kind, 5, 3, 1)
            ...
            assert all(
                ...
                for param, grad in zip(
                    model.params, model.grads, strict=True
                )
            )
