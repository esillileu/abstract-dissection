"""Dependency-neutral events emitted by the event-based trainers."""
...
class UpdateEvent:
    ...
    ``loss`` remains a backend scalar so a GPU executor can buffer it without
    synchronizing its stream for every update.
    """
...
class SourceObjectiveSample:
    """Pre-update objective retained only to reconstruct a book source curve."""
...
class TrainingWindowEvent:
    """Executor-owned timing window closed at a probe, epoch end, or terminal."""
