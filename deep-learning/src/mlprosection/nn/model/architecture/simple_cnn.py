...
class SimpleCNN(Sequential):
    def __init__(self, *, input_channels: int = 1, image_size: int = 28, num_classes: int = 10, conv_channels: int = 30, kernel_size: int = 5, stride: int = 1, padding: int = 0, hidden_size: int = 100, initializer: str = "std:0.01") -> None:
        ...
        super().__init__(Conv2D(input_channels, conv_channels, (kernel_size, kernel_size), stride, padding, initializer=initializer), Relu(), MaxPool2D(2, 2), Flatten(), hidden, Relu(), output)
