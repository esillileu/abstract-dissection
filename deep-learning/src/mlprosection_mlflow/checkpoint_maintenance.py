...
def _report(command: str, *, apply: bool, entries: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "command": command,
    ...
    *,
    ...
    for name in experiments:
        ...
        for run in _canonical_seed_runs_by_completeness(
            ...
            checkpoint_root = repository_root / "exp" / name / "results/checkpoints" / run_key
            artifact_root = repository_root / "exp" / name / "results/mlflow_artifacts" / run_key
                ...
                if remote_digest is not None:
                    ...
                    if apply:
                        if source.is_dir():
                            ...
                            client.log_artifact(run_id, str(source), artifact_path="checkpoints")
                        ...
                        client.set_tag(run_id, f"checkpoint.{role}.sha256", local["digest"])
                entries.append({
                    "experiment": name,
                    "run_id": run_id,
                    "run_key": run_key,
            ...
            if apply and missing_roles:
                client.set_tag(run_id, "checkpoint.missing_roles", ",".join(missing_roles))
...
def _upload_pointer(
    ...
    *,
    ...
    if pointer_path is None:
        pointer_path = (
            f"generations/{source.name}" if source.is_dir() else source.name
        )
    ...
    *,
    ...
    for name in experiments:
        ...
        for run in _canonical_seed_runs(_runs(client, experiment.experiment_id)):
            run_key = run.data.tags["run.key"]
            local_root = repository_root / "exp" / name / "results/checkpoints" / run_key
            ...
            preserve_local.update(
                path.resolve()
                for path in (local_root / "latest.json", local_root / "best.json", local_root / "final.npz")
                if path.exists()
            )
    ...
    *,
    ...
    if csv_path.exists():
        ...
        role_manifests = {
            ...
            for role, item in (
                (role, roles.get(role)) for role in CHECKPOINT_ROLES
            )
        }
...
def _completeness(
    ...
    *,
    ...
    checkpoints_names = _direct_artifact_names(
        client, run.info.run_id, "checkpoints"
    )
...
def dedupe(
    ...
    *,
    ...
    for run_type, identity in (
        ...
        grouped: dict[str, list[Any]] = defaultdict(list)
        for run in runs:
            if run.data.tags.get("run.type") == run_type and run.data.tags.get(identity):
