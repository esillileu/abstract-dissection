    ...
    *,
    ...
    return list(
        client.search_runs(
            [experiment_id],
            run_view_type=(
                ViewType.ACTIVE_ONLY if active_only else ViewType.ALL
            ),
            max_results=50_000,
        )
    )
...
def reconcile_parent_links(
    ...
    *,
    ...
    for group_key, children in children_by_group.items():
        ...
        for run in children:
            current_ids = {
                tag: run.data.tags.get(tag) for tag in PARENT_TAGS
            }
            _reconcile_child(
                client,
                experiment_id,
...
def _reconcile_child(
    ...
    *,
    ...
    if (
        primary_id in runs_by_id
        and current_ids[PARENT_TAGS[1]] == primary_id
    ):
    ...
    entries.append({
        ...
        "candidate_parent_ids": [
            candidate.info.run_id for candidate in candidates
        ],
