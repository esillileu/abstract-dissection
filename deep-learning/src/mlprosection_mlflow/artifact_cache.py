...
def tracking_uri_key(uri: str) -> str:
    ...
    normalized = urlunsplit((parsed.scheme.lower(), netloc, parsed.path, parsed.query, ""))
...
class MlflowArtifactCache:
    ...
    def path(self, run_id: str, artifact_path: str) -> Path:
        parts = tuple(part for part in Path(artifact_path).parts if part not in {"", "."})
...
def _validate(run_id: str, parts: tuple[str, ...]) -> None:
    values = (run_id, *parts)
    if not parts or any(not value or value in {".", ".."} or Path(value).name != value for value in values):
