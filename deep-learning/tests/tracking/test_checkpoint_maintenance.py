...
from mlprosection_mlflow.checkpoint_maintenance import (
    checkpoint_backfill,
    checkpoint_prune,
    dedupe,
    path_digest,
...
def test_dedupe_prefers_completeness_over_recency_and_reparents(
    ...
    seed_entry = next(item for item in report["entries"] if item["run_type"] == "seed_trial")
    ...
    assert client.get_run(complete.info.run_id).data.tags[
        "mlflow.parentRunId"
    ] == canonical_parent
