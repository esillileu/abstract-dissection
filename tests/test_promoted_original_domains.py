...
from dlfs.ds1.catalog import ORIGINAL as DS1_ORIGINAL
from dlfs.ds2.catalog import ORIGINAL as DS2_ORIGINAL
...
def test_default_catalog_sizes_include_ds2_original_word2vec() -> None:
    ds1 = Planner(DS1_ORIGINAL).build(
        RunSelection(all_experiments=True), RunOptions(device="cpu")
    )
    ds2 = Planner(DS2_ORIGINAL).build(
        RunSelection(all_experiments=True), RunOptions(device="cpu")
    )
...
def test_ds2_e05_uses_upstream_better_rnnlm_recipe() -> None:
    spec = DS2_ORIGINAL.load_run_spec(
        Path("studies/dlfs/src/dlfs/ds2/config/original/e05_better_rnnlm.yaml"),
...
def test_atomic_seed_override_and_seed_first_order() -> None:
    plans = Planner(DS1_ORIGINAL).build(
        RunSelection(
            experiment_ids=("e01",), atomic_run_ids=("OPT-SGD",), seed_values="2,1"
        ),
        RunOptions(
            device="cpu",
...
def test_upstream_import_context_restores_modules() -> None:
    from dlfs.ds1.original.run.common import source_imports

    sentinel = object()
    previous = sys.modules.get("common", sentinel)
    source = (
        RuntimePaths.from_environment().reference("dlfs1-book") / "source"
    ).resolve()
...
def test_ds2_e08_requires_matching_seed_source() -> None:
    spec = DS2_ORIGINAL.load_run_spec(
        Path("studies/dlfs/src/dlfs/ds2/config/original/e08_attention.yaml"),
