...
def test_cache_expected_identity_invalidates_source_or_config_change(
    ...
    assert not cache_is_valid(target, {"source_hashes": {"source.py": "different"}})
...
def test_ds2_original_summary_reads_only_original_cache(
    ...
    from dlfs.ds2.original.summary import summarize

    root = tmp_path / "original"
    target = root / "data" / "e01" / "dlfs2.ch03.toy-cbow-full-softmax"
    staging = staging_directory(target)
    save_csv(
        staging / "metrics.csv",
        (
...
def test_ds1_renderer_uses_only_persisted_fixture(
    ...
    assert [path.name for path in outputs] == ["e09_optimizer_compare_naive.png"]
    assert outputs[0].is_file()
    assert not any(".original.run" in name for name in newly_imported)
    assert not any(
        name == "dataset" or name.startswith("dataset.") for name in newly_imported
    )
...
def test_to_host_converts_a_sequence_of_cupy_scalars(
    ...
    from dlfs.original_runtime.cache_protocol import to_host
...
def test_parameter_count_deduplicates_shared_tensor_references() -> None:
    from dlfs.original_runtime.measurement import count_parameters
...
def test_ds2_original_trials_all_use_cupy() -> None:
    from dlfs.ds2.original.run.api import trials_for

    trials = trials_for(["e01", "e02", "e03", "e04", "e05", "e06", "e07", "e08"])
    by_experiment = {}
...
def test_ds2_ch07_compatibility_alias_supports_peeky_import() -> None:
    from dlfs.ds2.original.run.api import UPSTREAM
    from dlfs.ds2.original.run.common import (
